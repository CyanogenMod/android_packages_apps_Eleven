...
public class MusicPlaybackService extends Service {
            ...
            public boolean onMediaButtonEvent(@NonNull Intent mediaButtonIntent) {
                if (Intent.ACTION_MEDIA_BUTTON.equals(mediaButtonIntent.getAction())) {
                    ...
                    if (ke != null
                            ...
                            && ke.getKeyCode() == KeyEvent.KEYCODE_HEADSETHOOK) {
    ...
    private void handleCommandIntent(Intent intent) {
        ...
        if (CMDNEXT.equals(command) || NEXT_ACTION.equals(action)) {
            ...
            cycleShuffle();
        } else if (CMDHEADSETHOOK.equals(action)) {
    ...
    private static final class MusicPlayerHandler extends Handler {
        ...
        private long mLastHeadsetHookClickTime = 0;
            ...
            synchronized (service) {
                switch (msg.what) {
                    ...
                    case HEADSET_HOOK_EVENT: {
                        ...
                        if (eventTime - mLastHeadsetHookClickTime >= DOUBLE_CLICK_TIMEOUT) {
                            mHeadsetHookClickCounter = 0;
                        }

                        mHeadsetHookClickCounter++;
                        if (D) Log.d(TAG, "Got headset click, count = " + mHeadsetHookClickCounter);
                        removeMessages(HEADSET_HOOK_MULTI_CLICK_TIMEOUT);

                        Message timeoutMsg = obtainMessage(HEADSET_HOOK_MULTI_CLICK_TIMEOUT,
                                mHeadsetHookClickCounter, 0);

                        long delay = mHeadsetHookClickCounter < 3 ? DOUBLE_CLICK_TIMEOUT : 0;
                        if (mHeadsetHookClickCounter >= 3) {
                            mHeadsetHookClickCounter = 0;
                        }
                        mLastHeadsetHookClickTime = eventTime;
                        sendMessageDelayed(timeoutMsg, delay);
                        break;
                    }
                    case HEADSET_HOOK_MULTI_CLICK_TIMEOUT:
                        if (D) Log.d(TAG, "Handling headset click, count = " + msg.arg1);
                        switch (msg.arg1) {
